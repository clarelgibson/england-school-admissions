...
The source data for performance is stored in several CSV files on the Google Drive. In this case we will need to read each file in separately, check if the column headings are the same, and then bind same columns from each source file into a single data frame.
