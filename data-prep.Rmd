...
```{r offers-extract-la-name}
offers_la <- offers %>% 
  select(urn, year, la_code, la_name, region_code, region_name) %>% 
  distinct() %>% 
  arrange(urn, -year, la_code, la_name) %>% 
  group_by(urn) %>% 
  slice_head(n = 1) %>% 
  ungroup() %>%
  ...
  select(urn, master_urn, year, la_code, la_name,
         region_code, region_name) %>% 
  mutate(la_code = case_when(urn == 121820 ~ 941,
                             urn == 121883 ~ 941,
                             urn == 122020 ~ 940,
                             urn == 142147 ~ 940,
                             TRUE ~ la_code),
         la_name = case_when(urn == 121820 ~ "West Northamptonshire",
                             urn == 121883 ~ "West Northamptonshire",
                             urn == 122020 ~ "North Northamptonshire",
                             urn == 142147 ~ "North Northamptonshire",
                             TRUE ~ la_name)) %>% 
  select(la_code, la_name, region_code, region_name) %>% 
  distinct()
```
...
offers <- offers %>% 
  mutate(la_code = case_when(la_code == 837 ~ 839,
                             la_code == 835 ~ 838,
                             la_code == 836 ~ 839,
                             la_name == "Wokingham" ~ 872,
                             la_name == "Norfolk" ~ 926,
                             TRUE ~ la_code),
         la_name = case_when(grepl("Bournemouth|Poole", la_name)
                             ~ "Bournemouth, Christchurch and Poole",
                             grepl("Bristol", la_name)
                             ~ "Bristol, City of",
                             grepl("Durham", la_name)
                             ~ "County Durham",
                             grepl("Herefordshire", la_name)
