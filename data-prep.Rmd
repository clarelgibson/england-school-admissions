...
# Define the performance data to include
fct_school_year_perf <- performance %>% 
...
fct_school_year <- offers %>% 
  ...
  full_join(fct_school_year_perf) %>% 
...
fct_school <- info %>% 
  ...
  left_join(select(dim_la,
                   ...
                   la_key)) %>% 
  # join phase key
  left_join(select(fct_school_year,
                   school_key,
                   phase_key)) %>% 
  mutate(phase_key = if_else(is.na(phase_key),
                             2,
                             phase_key)) %>% 
  # select final columns
  select(school_key,
