...
This file is the same as the one above except that here, the data is aggregated at the local authority level. This file will be important as our single source of truth for local authority-level measures (e.g. the number of preferences allowed on the application form).

```{r offersla-read}
# Read in the source data for offers_la
src_offersla_path <- "https://drive.google.com/file/d/12jONhUf3xSZdRbsUE_4tZRA014sIHO_D/view?usp=sharing"
src_offersla <- read_csv_gdrive(src_offersla_path)
```
...
Now, let's review the unique values of LA code and name in the `info` df.
...
### By Local Authority
From the `src_offersla` we only need to keep the identifiers for local authority, year and phase as well as the number of preferences. We should also filter the source data to keep only data at the `geographic_level` of "Local authority".

```{r offersla-select-cols}
# Select the required columns from offers_la
offers_la <- src_offersla %>% 
...
Now let's review the entries for the local authority identifiers.
...
There are some inconsistencies in the `number_of_preferences` recorded for each Local Authority. For each group of year and local authority, the number of preferences value should be the same. We need to clean this so that the most common occurence of the value for `number_of_preferences` is used for all LA within a given year.

```{r offers-clean-num-prefs}
# View the current state
offers_prefs <- offers %>%
  group_by(year, la_code) %>% 
  summarise(n_distinct_prefs = n_distinct(number_of_preferences)) %>% 
  filter(n_distinct_prefs > 1)

offers %>% 
  filter(year %in% offers_prefs$year,
         la_code %in% offers_prefs$la_code) %>% 
  count(year, la_code, number_of_preferences)
```
